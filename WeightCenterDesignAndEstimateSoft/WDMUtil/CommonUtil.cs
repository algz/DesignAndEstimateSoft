...
namespace WeightCenterDesignAndEstimateSoft
{
    ...
    class CommonUtil
    {
        ...
        public string exportInertiaAssessXmlString(List<InertiaArithmetic> inertiaList)
        {
            string s = "<转动惯量计算结果列表>";
            foreach (InertiaArithmetic result in inertiaList)
            {
                s += exportReflectionObject(result);
            }
            return s + "</转动惯量计算结果列表>";
        }
        ...
        /// <param name="doc"></param>
        /// <returns></returns>
        public List<InertiaArithmetic> importInertiaXMLString(XmlDocument doc)
        {
            XmlNode rootNode = doc.SelectSingleNode("PRJ/重量重心设计工程/转动惯量计算结果列表");
            if (rootNode == null)
            {
                return null;
            }
            List<InertiaArithmetic> list = new List<InertiaArithmetic>();
            foreach (XmlNode node in rootNode.ChildNodes)
            {
                list.Add((InertiaArithmetic)importReflectionObject(node, typeof(InertiaArithmetic)));
            }
            return list;
        }
