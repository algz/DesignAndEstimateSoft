...
using XCommon.assessData;
using WeightCenterDesignAndEstimateSoft.Task.CoreEnvelopeAssessment;
using System.Text.RegularExpressions;

using Excel = Microsoft.Office.Interop.Excel;
using WeightCenterDesignAndEstimateSoft.Tool.GenerateReport;
...
namespace WeightCenterDesignAndEstimateSoft
{
    public partial class MainForm : Form
    {
        ...
        //重量设计结果集合
        private List<XCommon.WeightArithmetic> lstWeightArithmetic = null;
        //重量调整结果集合
        ...
        //重心包线设计结果集合
        private List<XCommon.CoreEnvelopeArithmetic> lstCoreEnvelopeDesign = null;
        //重量评估结果对象集合
        ...
        public List<InertiaArithmetic> lstInertiaArithmetic = null;
        ...
        public void BindProjectTreeData(DesignProjectData ProjectData)
        {
            ...
            if (ProjectData != null)
            {
                ...
                lstInertiaArithmetic = ProjectData.lstIntertiaArithmetic;
                ...
                //转动惯量计算结果列表
                if (lstInertiaArithmetic != null && lstInertiaArithmetic.Count > 0)
                {
                    TreeNode parentNode = new TreeNode("转动惯量计算结果列表");
                    projectNode.Nodes.Add(parentNode);

                    for (int i = 0; i < lstInertiaArithmetic.Count; i++)
                    {
                        TreeNode childNode = new TreeNode();
                        childNode.Name = lstInertiaArithmetic[i].DataName;
                        childNode.Text = lstInertiaArithmetic[i].DataName;
                        childNode.Tag = lstInertiaArithmetic[i];
                        childNode.ToolTipText = i.ToString();
                        parentNode.Nodes.Add(childNode);
                    }
                }
        ...
        private DesignProjectData GetResultData(string strFilePath)
        {
            ...
            //重量设计结果列表
            List<XCommon.WeightArithmetic> lstWeightArithmeticData = GetWeightDesignReusltData(doc);
            ...
            //重心包线设计结果列表
            List<XCommon.CoreEnvelopeArithmetic> lstCoreEnvelopeData = GetCoreEnvelopeDesignReustData(doc);
            projectData.lstCoreEnvelopeDesign = lstCoreEnvelopeData;

            //重心包线剪裁结果列表
            List<XCommon.CoreEnvelopeCutResultData> lstCoreEnvelopteCutData = GetCoreEnvelopteCutData(doc);
            ...
            //转动惯量计算结果列表
            List<InertiaArithmetic> lstInertia = GetInertiaReusltData(doc);
            projectData.lstIntertiaArithmetic = lstInertia;
        ...
        /// <returns></returns>
        private List<XCommon.WeightArithmetic> GetWeightDesignReusltData(XmlDocument doc)
        {
            string path = string.Empty;
            XmlNode node = null;

            //重量设计结果列表
            List<XCommon.WeightArithmetic> lstWeightArithmeticData = new List<XCommon.WeightArithmetic>();
            //重量设计结果
            ...
            {
                XCommon.WeightArithmetic arithmetic = new XCommon.WeightArithmetic();
                ...
                //lstWeightFormula
                List<XCommon.WeightFormula> lstWeightFormula = new List<XCommon.WeightFormula>();
                ...
                {
                    XCommon.WeightFormula formula = new XCommon.WeightFormula();

                    formula.NodePath = lstChildNode[n].ChildNodes[0].InnerText;
                    formula.Formula = lstChildNode[n].ChildNodes[1].InnerText;

                    List<XCommon.ParaData> lstWeightPara = new List<XCommon.ParaData>();
                    ...
                    for (int i = 0; i < lstChildNode[n].ChildNodes[2].ChildNodes.Count; i++)
                    {
                        ...
                        string strEnName = lstChildNode[n].ChildNodes[2].ChildNodes[i].ChildNodes[1].InnerText;
                        string strUnit = lstChildNode[n].ChildNodes[2].ChildNodes[i].ChildNodes[2].InnerText;

                        ParaData wpGlobal = WeightArithmetic.FindGlobleParameters(strName, strUnit)[0];

                        XCommon.ParaData para = null;
                        if (wpGlobal == null)
                        {
                            para = new ParaData();

                            para.paraName = strName;
                            para.paraEnName = strEnName;
                            para.paraUnit = strUnit;
                            para.paraType = 10;
                            //para.paraValue = Convert.ToDouble(lstChildNode[n].ChildNodes[2].ChildNodes[i].ChildNodes[4].InnerText);
                            para.strRemark = lstChildNode[n].ChildNodes[2].ChildNodes[i].ChildNodes[5].InnerText;

                            ParaData temp10wp = new ParaData(para);
                            ParaData.GetWeightParameterList()[10].Add(temp10wp);
                            para.paraValue = Convert.ToDouble(lstChildNode[n].ChildNodes[2].ChildNodes[i].ChildNodes[4].InnerText);
        ...
        /// <param name="doc"></param>
        /// <returns></returns>
        private List<XCommon.WeightAdjustmentResultData> GetAdjustmentResultData(XmlDocument doc)
        {
            string path = string.Empty;
            XmlNode node = null;

            //重量设计结果列表
            List<XCommon.WeightAdjustmentResultData> lstAdjustData = new List<XCommon.WeightAdjustmentResultData>();
            //重量设计结果
        ...
        /// <param name="doc"></param>
        /// <returns></returns>
        private List<XCommon.CoreEnvelopeArithmetic> GetCoreEnvelopeDesignReustData(XmlDocument doc)
        {
            string path = string.Empty;
            XmlNode node = null;

            List<XCommon.CoreEnvelopeArithmetic> lstCoreEnvelopeDesignData = new List<XCommon.CoreEnvelopeArithmetic>();
            ...
            for (int m = 0; m < node.ChildNodes.Count; m++)
            {
                XCommon.CoreEnvelopeArithmetic arithmetic = new XCommon.CoreEnvelopeArithmetic();
                //重心包线设计数据名称
                arithmetic.DataName = node.ChildNodes[m].ChildNodes[0].InnerText;
                ...
                List<XCommon.NodeFormula> lstFormulaList = new List<XCommon.NodeFormula>();
                ...
                {
                    XCommon.NodeFormula formula = new XCommon.NodeFormula();
                    formula.NodeName = lstNode[i].ChildNodes[0].InnerText;

                    XCommon.WeightFormula XFormula = formula.XFormula;
                    ...
                    for (int k = 0; k < lstXNode.Count; k++)
                    {
                        ...
                        if (lstXNode[k].LocalName == "参数列表")
                        {
                            ...
                            foreach (XmlNode paraNode in lstXNode[k].ChildNodes)
                            {
                                ...
                                string enname = paraNode.ChildNodes[1].InnerText;
                                string unit = paraNode.ChildNodes[2].InnerText;
                                ParaData wpGlobal = WeightArithmetic.FindGlobleParameters(name, unit)[0];
                                ...
                                if (wpGlobal == null)
                                {
                                    wp = new ParaData();
                                    wp.paraName = name;
                                    wp.paraEnName = enname;
                                    wp.paraUnit = unit;
                                    wp.paraType = 10;
                                    //wp.paraValue = Convert.ToDouble(paraNode.ChildNodes[4].InnerText);
                                    wp.strRemark = paraNode.ChildNodes[5].InnerText;


                                    ParaData temp10wp = new ParaData(wp);
                                    ParaData.GetWeightParameterList()[10].Add(temp10wp);
                                    wp.paraValue = Convert.ToDouble(paraNode.ChildNodes[4].InnerText);
                    ...
                    XCommon.WeightFormula YFormula = formula.YFormula;
        ...
        private List<string> GetFileContent()
        {
            ...
            if (designProjectData != null)
            {
                ...
                //转动惯量计算结果
                List<string> lstInertiaConetent = GetInertiaContent();
                foreach (string strConetent in lstInertiaConetent)
                {
                    lstContent.Add(strConetent);
                }
                ...
                foreach (WeightAdjustmentResultData adjustResult in lstResult)
                {
                    ...
                    if (adjustResult.checkFactor != null && adjustResult.checkFactor.Count > 0)
                    {
                        foreach (ParaData data in adjustResult.checkFactor)
                        {
                            ...
                            lstContent.Add(str);
                            str = CommonFunction.mStrModifyToString8(7) + "<参数英文名称>" + data.paraEnName + "</参数英文名称>";
                            lstContent.Add(str);
        ...
        public void SetWeightDesignTab()
        {
            ...
            XCommon.WeightArithmetic weightArithmetic = GetWeightArithmetic();
            SetWeightDesignReuslt(weightArithmetic);

            tabControlWork.SelectedTab = tabPageWeightDesign;
        }

        public void SetWeightDesignTab(XCommon.WeightArithmetic weightArithmetic, int index)
        {
            if (tabControlWork.TabPages.Contains(tabPageWeightDesign) == false)
            {
                tabControlWork.TabPages.Add(tabPageWeightDesign);
        ...
        private List<string> GetDesignResultContent()
        {
            ...
            List<XCommon.WeightArithmetic> lstResult = designProjectData.lstWeightArithmetic;
            if (lstResult != null && lstResult.Count > 0)
            {
                string str = string.Empty;
                ...
                foreach (WeightArithmetic weightDesign in lstResult)
                {
                    ...
                    List<XCommon.ParaData> lstParaData = weightDesign.GetParaList();
                    ...
                    for (int j = 0; j < lstParaData.Count; j++)
                    {
                        ...
                        lstContent.Add(str);
                        str = CommonFunction.mStrModifyToString8(6) + "<参数英文名称>" + lstParaData[j].paraEnName + "</参数英文名称>";
                        lstContent.Add(str);
                        ...
                        lstContent.Add(str);
                        str = CommonFunction.mStrModifyToString8(6) + "<参数备注>" + lstParaData[j].strRemark + "</参数备注>";
                        lstContent.Add(str);
        ...
        /// </summary>
        /// <returns></returns>
        private List<string> GetDesignWeightFormulaContent(XCommon.WeightArithmetic arithmetic)
        {
            List<string> lstContent = new List<string>();
            string str = string.Empty;
            ...
            List<XCommon.WeightFormula> lstFormula = arithmetic.FormulaList;

            foreach (XCommon.WeightFormula formula in lstFormula)
            {
                ...
                foreach (XCommon.ParaData para in formula.ParaList)
                {
                    lstContent.Add(CommonFunction.mStrModifyToString8(9) + "<参数>");

                    str = CommonFunction.mStrModifyToString8(10) + "<参数名称>" + para.paraName + "</参数名称>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(10) + "<参数英文名称>" + para.paraEnName + "</参数英文名称>";
                    lstContent.Add(str);
                    ...
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(10) + "<参数备注>" + para.strRemark + "</参数备注>";
                    lstContent.Add(str);
        ...
        private XCommon.WeightArithmetic GetWeightArithmetic()
        {
            XCommon.WeightArithmetic weightArithmetic = null;
            for (int i = 0; i < lstWeightArithmetic.Count; i++)
            {
                if (selNode != null && selNode.Level == 2 && selNode.ToolTipText == i.ToString())
                {
                    weightArithmetic = new XCommon.WeightArithmetic();
        ...
        /// 设置重量设计结果
        /// </summary>
        public void SetWeightDesignReuslt(XCommon.WeightArithmetic weightArithmetic)
        {
        ...
        public void SetCoreEnvelopeDesignTab()
        {
            ...
            XCommon.CoreEnvelopeArithmetic coreEnvelope = GetCoreEnvelopeDesign();
            SetCoreEnvelopeDesignResult(coreEnvelope);
            tabControlWork.SelectedTab = tabPageEnvelopeDesign;
        }

        public void SetCoreEnvelopeDesignTab(XCommon.CoreEnvelopeArithmetic coreEnvelope, int index)
        {
            if (tabControlWork.TabPages.Contains(tabPageEnvelopeDesign) == false)
            {
                tabControlWork.TabPages.Add(tabPageEnvelopeDesign);
        ...
        private XCommon.CoreEnvelopeArithmetic GetCoreEnvelopeDesign()
        {
            XCommon.CoreEnvelopeArithmetic coreEnvelope = null;
        ...
        /// 设置重量设计结果
        /// </summary>
        public void SetCoreEnvelopeDesignResult(XCommon.CoreEnvelopeArithmetic coreEnvelope)
        {
        ...
        /// 绑定重心包线数据
        /// </summary>
        private void BindCoreEnvelopeData(XCommon.CoreEnvelopeArithmetic coreEnvelope)
        {
        ...
        private DataTable GetTableCoreEnvelopteData(XCommon.CoreEnvelopeArithmetic coreEnvelope)
        {
            ...
            if (table.Columns.Count > 0)
            {
                ...
                {
                    foreach (XCommon.NodeFormula formula in coreEnvelope.FormulaList)
                    {
        ...
        private List<string> GetListCoreEnvelopeDesign(XCommon.CoreEnvelopeArithmetic coreEnvelope)
        {
        ...
        private void SetCoreEnvelopeDesignGridView(XCommon.CoreEnvelopeArithmetic coreEnvelope)
        {
        ...
        private List<string> GetEnvelopeDesignResultContent()
        {
            ...
            List<XCommon.CoreEnvelopeArithmetic> lstResult = designProjectData.lstCoreEnvelopeDesign;
            if (lstResult != null && lstResult.Count > 0)
            {
                string str = string.Empty;
                ...
                foreach (CoreEnvelopeArithmetic coreEnvelope in lstResult)
                {
                    ...
                    List<XCommon.ParaData> lstPara = coreEnvelope.GetParaList();
                    if (lstPara != null && lstPara.Count > 0)
                    {
                        ...
                        for (int j = 0; j < lstPara.Count; j++)
                        {
                            ...
                            lstContent.Add(str);
                            str = CommonFunction.mStrModifyToString8(6) + "<参数英文名称>" + lstPara[j].paraEnName + "</参数英文名称>";
                            lstContent.Add(str);
                            ...
                            lstContent.Add(str);
                            str = CommonFunction.mStrModifyToString8(6) + "<参数备注>" + lstPara[j].strRemark + "</参数备注>";
                            lstContent.Add(str);
                    ...
                    List<XCommon.NodeFormula> lstNodeFormula = coreEnvelope.FormulaList;
                    ...
                    if (lstNodeFormula != null && lstNodeFormula.Count > 0)
                    {
                        ...
                        foreach (XCommon.NodeFormula data in lstNodeFormula)
                        {
                            ...
                            List<XCommon.ParaData> lstXParaData = data.XFormula.ParaList;

                            if (lstXParaData != null && lstXParaData.Count > 0)
                            {
                                lstContent.Add(CommonFunction.mStrModifyToString8(8) + "<参数列表>");

                                foreach (XCommon.ParaData para in lstXParaData)
                                {
                                    lstContent.Add(CommonFunction.mStrModifyToString8(9) + "<参数>");
                            ...
                            List<XCommon.ParaData> lstYParaData = data.YFormula.ParaList;

                            if (lstYParaData != null && lstYParaData.Count > 0)
                            {
                                lstContent.Add(CommonFunction.mStrModifyToString8(8) + "<参数列表>");

                                foreach (XCommon.ParaData para in lstYParaData)
                                {
                                    lstContent.Add(CommonFunction.mStrModifyToString8(9) + "<参数>");
        ...
        /// </summary>
        /// <returns></returns>
        private List<XCommon.CoreEnvelopeCutResultData> GetCoreEnvelopteCutData(XmlDocument doc)
        {
            string path = string.Empty;
            XmlNode node = null;

            List<XCommon.CoreEnvelopeCutResultData> lstCoreEnvelopeCutData = new List<CoreEnvelopeCutResultData>();
            ...
            {
                XCommon.CoreEnvelopeCutResultData cutData = new CoreEnvelopeCutResultData(0);
        ...
        private XCommon.CoreEnvelopeCutResultData GetCoreEnvelopeCutResult()
        {
        ...
        private void SetCoreEnvelopeCutGridView(XCommon.CoreEnvelopeCutResultData cutData, string strType)
        {
        ...
        private List<string> GetListCoreEnvelopeCut(XCommon.CoreEnvelopeCutResultData cutData, string strType)
        {
        ...
        public void SetInertiaTab(InertiaArithmetic result)
        {
            if (tabControlWork.TabPages.Contains(tabPageInertia) == false)
            {
                tabControlWork.TabPages.Add(tabPageInertia);
            }
            tabPageInertia.ToolTipText = selNode.ToolTipText;
            tabPageInertia.Text = result.DataName;
            SetInertiaResult(result);
            tabControlWork.SelectedTab = tabPageInertia;
        }

        public void SetInertiaTab(InertiaArithmetic result, int index)
        {
            if (tabControlWork.TabPages.Contains(tabPageInertia) == false)
            {
                tabControlWork.TabPages.Add(tabPageInertia);
            ...
            tabPageInertia.ToolTipText = index.ToString();
            tabPageInertia.Text = result.DataName;
            SetInertiaResult(result);
            tabControlWork.SelectedTab = tabPageInertia;
            ...
            foreach (TreeNode parentNode in rootNode.Nodes)
            {
                if (parentNode.Text == "转动转动惯量设计结果列表")
                {
                    foreach (TreeNode node in parentNode.Nodes)
                    {
                        if (node.ToolTipText == index.ToString())
                        {
                            treeViewData.SelectedNode = node;
                            selNode = node;
                        }
        ...
        private void SetInertiaResult(InertiaArithmetic result)
        {
            if (result != null)
            {
                txtIneritaName.Text = result.DataName;

                BindGridInertiaData(result);
            }

        }

        private void BindGridInertiaData(InertiaArithmetic result)
        {
            if (result.FormulaList != null && result.FormulaList.Count > 0)
            {
                gridInertiaData.Columns.Clear();
                gridInertiaData.DataSource = null;


                DataGridViewColumn column1 = new DataGridViewColumn();
                gridInertiaData.Columns.Add("column1", "名称");
                gridInertiaData.Columns.Add("column2", "数值");

                gridInertiaData.Columns[1].ValueType = System.Type.GetType("System.Decimal");
                gridInertiaData.Columns[0].SortMode = DataGridViewColumnSortMode.NotSortable;
                gridInertiaData.Columns[0].ReadOnly = true;
                gridInertiaData.Columns[1].SortMode = DataGridViewColumnSortMode.NotSortable;
                gridInertiaData.Rows.Add(result.FormulaList.Count);

                for (int i = 0; i < gridInertiaData.Rows.Count; i++)
                {
                    gridInertiaData.Rows[i].Cells[0].Value = result.FormulaList[i].NodePath;
                    gridInertiaData.Rows[i].Cells[1].Value = result.FormulaList[i].Value;
                }
            }
        }

        private XCommon.InertiaArithmetic GetInertiaData()
        {
            XCommon.InertiaArithmetic result = null;
            for (int i = 0; i < lstInertiaArithmetic.Count; i++)
            {
                if (selNode != null && selNode.Level == 2 && selNode.ToolTipText == i.ToString())
                {
                    result = new XCommon.InertiaArithmetic();
                    result = lstInertiaArithmetic[i];
                }
            }
            return result;
        }
        ...
        /// </summary>
        /// <returns></returns>
        private List<string> GetInertiaFormulaContent(XCommon.InertiaArithmetic arithmetic)
        {
            List<string> lstContent = new List<string>();
            string str = string.Empty;

            lstContent.Add(CommonFunction.mStrModifyToString8(5) + "<公式列表>");

            List<XCommon.WeightFormula> lstFormula = arithmetic.FormulaList;

            foreach (XCommon.WeightFormula formula in lstFormula)
            {
                lstContent.Add(CommonFunction.mStrModifyToString8(6) + "<公式>");

                str = CommonFunction.mStrModifyToString8(7) + "<节点名称>" + formula.NodePath + "</节点名称>";
                lstContent.Add(str);
                str = CommonFunction.mStrModifyToString8(7) + "<公式>" + formula.Formula + "</公式>";
                lstContent.Add(str);

                lstContent.Add(CommonFunction.mStrModifyToString8(7) + "<参数列表>");
                foreach (XCommon.ParaData para in formula.ParaList)
                {
                    lstContent.Add(CommonFunction.mStrModifyToString8(8) + "<参数>");

                    str = CommonFunction.mStrModifyToString8(9) + "<参数名称>" + para.paraName + "</参数名称>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(9) + "<参数英文名称>" + para.paraEnName + "</参数英文名称>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(9) + "<参数单位>" + para.paraUnit + "</参数单位>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(9) + "<参数类型>" + para.paraType.ToString() + "</参数类型>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(9) + "<参数数值>" + para.paraValue.ToString() + "</参数数值>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(9) + "<参数备注>" + para.strRemark + "</参数备注>";
                    lstContent.Add(str);
                ...
                lstContent.Add(CommonFunction.mStrModifyToString8(7) + "</参数列表>");
                str = CommonFunction.mStrModifyToString8(7) + "<结果数值>" + formula.Value + "</结果数值>";
                lstContent.Add(str);
        ...
        private List<string> GetInertiaContent()
        {
            List<string> lstContent = new List<string>();
            List<InertiaArithmetic> lstTempInertia = designProjectData.lstIntertiaArithmetic;

            if (lstTempInertia != null && lstTempInertia.Count > 0)
            {
                string str = string.Empty;
                lstContent.Add(CommonFunction.mStrModifyToString8(2) + "<转动惯量计算结果列表>");

                foreach (InertiaArithmetic result in lstTempInertia)
                {
                    lstContent.Add(CommonFunction.mStrModifyToString8(3) + "<转动惯量计算结果>");
                    str = CommonFunction.mStrModifyToString8(4) + "<转动惯量计算数据名称>" + result.DataName + "</转动惯量计算数据名称>";
                    lstContent.Add(str);

                    lstContent.Add(CommonFunction.mStrModifyToString8(4) + "<参数列表>");

                    List<XCommon.ParaData> lstParaData = result.GetParaList();
                    for (int j = 0; j < lstParaData.Count; j++)
                    {
                        lstContent.Add(CommonFunction.mStrModifyToString8(5) + "<参数>");
                    ...
                    //-----------------------------------算法－－－－－－－－－－－－－－－－－－－－－－－//
                    lstContent.Add(CommonFunction.mStrModifyToString8(4) + "<转动惯量算法>");
                    str = CommonFunction.mStrModifyToString8(5) + "<转动惯量算法名称>" + result.Name + "</转动惯量算法名称>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(5) + "<转动惯量算法创建时间>" + result.CreateTime + "</转动惯量算法创建时间>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(5) + "<转动惯量算法最后修改时间>" + result.LastModifyTime + "</转动惯量算法最后修改时间>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(5) + "<转动惯量算法备注>" + result.Remark + "</转动惯量算法备注>";
                    lstContent.Add(str);

                    //公式列表
                    List<string> lstInertiaFormulaContent = GetInertiaFormulaContent(result);
                    foreach (string strContent in lstInertiaFormulaContent)
                    {
                        lstContent.Add(strContent);
                    }
                    lstContent.Add(CommonFunction.mStrModifyToString8(4) + "</转动惯量算法>");
        ...
        /// <param name="doc"></param>
        /// <returns></returns>
        private List<XCommon.InertiaArithmetic> GetInertiaReusltData(XmlDocument doc)
        {
            string path = string.Empty;
            XmlNode node = null;

            List<XCommon.InertiaArithmetic> lstInertiaData = new List<XCommon.InertiaArithmetic>();
            path = "PRJ/重量重心设计工程/转动惯量计算结果列表";
            node = doc.SelectSingleNode(path);

            if (node == null)
            {
                return lstInertiaData;
            }

            for (int m = 0; m < node.ChildNodes.Count; m++)
            {
                XCommon.InertiaArithmetic arithmetic = new XCommon.InertiaArithmetic();
                ...
                arithmetic.Remark = node.ChildNodes[m].ChildNodes[2].ChildNodes[3].InnerText;

                //lstWeightFormula
                List<XCommon.WeightFormula> lstWeightFormula = new List<XCommon.WeightFormula>();

                XmlNodeList lstChildNode = node.ChildNodes[m].ChildNodes[2].ChildNodes[4].ChildNodes;
                ...
                for (int n = 0; n < lstChildNode.Count; n++)
                {
                    ...
                    arithmetic.FormulaList[n] = formula;
                }

                lstInertiaData.Add(arithmetic);
            }

            return lstInertiaData;
        }

        private List<string> GetInertiaResultFlieContent()
        {
            List<string> lstContent = new List<string>();
            string str = string.Empty;
            if (gridInertiaData.Rows.Count > 0)
            {
                lstContent.Insert(0, "<?xml version=" + "\"1.0\"" + " encoding=" + "\"gb2312\"" + " ?> ");

                lstContent.Add("<转动惯量计算结果列表>");
                for (int i = 0; i < gridInertiaData.Rows.Count; i++)
                {
                    str = CommonFunction.mStrModifyToString8(1) + "<转动惯量计算结果>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(2) + "<名称>" + gridInertiaData.Rows[i].Cells[0].Value.ToString() + "</名称>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(2) + "<单位>" + "千克米平方" + "</单位>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(2) + "<数值>" + gridInertiaData.Rows[i].Cells[1].Value.ToString() + "</数值>";
                    lstContent.Add(str);
                    str = CommonFunction.mStrModifyToString8(1) + "</转动惯量计算结果>";
                    lstContent.Add(str);
                }
        ...
        private void ExportInertiaDataToExcel(string strFile)
        {
            ...
            try
            {
                ...
                DataCell.Value2 = "名称";
                DataCell.Next.Value2 = "数值";
                DataCell.Next.Next.Value2 = "单位";

                for (int i = 0; i < gridInertiaData.Rows.Count; ++i)
                {
                    string cellid = "A" + (i + 2).ToString();
                    DataCell = wSheet.get_Range(cellid, cellid);
                    DataCell.Value2 = gridInertiaData.Rows[i].Cells[0].Value.ToString();
                    DataCell.Next.Value2 = gridInertiaData.Rows[i].Cells[1].Value.ToString();
                    DataCell.Next.Next.Value2 = "千克米平方";
                }
                ...
                //保存工作簿    
                wBook.SaveAs(strFile, Excel.XlFileFormat.xlWorkbookNormal, missing, missing, missing, missing, Excel.XlSaveAsAccessMode.xlNoChange, missing, missing, missing, missing, missing);
        ...
        {
            //文件中参数列表
            List<ParaData> lstWeightParaData = new List<ParaData>();

            //参数表中的参数
            List<ParaData> lstPara = WeightEstimateForm.GetListParaData();

            List<List<ParaData>> lstWeightPara = ParaData.GetWeightParameterList();
            if (lstWeightPara != null && lstWeightPara.Count > 0)
            {
                foreach (List<ParaData> lstParaData in lstWeightPara)
                {
            ...
            {
                bool IsExit = false;
                foreach (ParaData data in lstWeightParaData)
                {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ToolStripMenuItemInertiaDesign_Click(object sender, EventArgs e)
        {
            InertiaDesignForm form = new InertiaDesignForm(this, null);
            form.ShowDialog();
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void toolStripMenuItemInertiaAtithmeticManage_Click(object sender, EventArgs e)
        {
            InertiaArithmeticManageForm form = new InertiaArithmeticManageForm(this);
            form.ShowDialog();
        }
        ...
        private void treeViewData_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (selNode.Level == 2 && selNode.Parent.Text == "转动惯量计算结果列表")
            {
                SetInertiaTab((InertiaArithmetic)selNode.Tag);
            }
        }
        ...
        private void treeViewData_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            if (selNode.Level == 2 && selNode.Parent.Text == "重心包线设计结果列表")
            {
                XCommon.CoreEnvelopeArithmetic coreArithmetic = GetCoreEnvelopeDesign();
            ...
            if (selNode.Level == 2 && selNode.Parent.Text == "转动惯量计算结果列表")
            {
                InertiaArithmetic result = GetInertiaData();
                InertiaDesignForm form = new InertiaDesignForm(this, result);
                form.ShowDialog();
            }
        }
        ...
        private void toolStripMenuItemDeleteWeightDesignData_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                if (selNode.Level == 1)
                {
                    ...
                    if (selNode.Text == "转动惯量计算结果列表")
                    {
                        designProjectData.lstIntertiaArithmetic.Clear();
                    }
                }
                if (selNode.Level == 2)
                {
                    ...
                    if (selNode.Parent.Text == "转动惯量计算结果列表")
                    {
                        InertiaArithmetic resultInertia = (InertiaArithmetic)selNode.Tag;
                        designProjectData.lstIntertiaArithmetic.Remove(resultInertia);
                    }
                }
        ...
        private void tabControlWork_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (tabPage != null && rootNode != null)
            {
                ...
                //转动惯量设计
                if (tabPage.Name ==tabPageInertia.Name)
                {
                    foreach (TreeNode parentNode in rootNode.Nodes)
                    {
                        if (parentNode.Text == "转动惯量计算结果列表")
                        {
                            foreach (TreeNode node in parentNode.Nodes)
                            {
                                if (node.ToolTipText == tabPage.ToolTipText)
        ...
        private void gridInertiaData_DataError(object sender, System.Windows.Forms.DataGridViewDataErrorEventArgs e)
        {
            MessageBox.Show("非法数字");
            e.Cancel = true;
        }
        ...
        {
            if (this.designProjectData == null)
            {
                return;
            }
            new GenerateReportForm().Show(this);
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnExportInertiaFile_Click(object sender, EventArgs e)
        {
            if (gridInertiaData.Rows.Count == 0)
            {
                XLog.Write("没有数据不能导出");
            ...
            dlg.FilterIndex = 0;
            dlg.RestoreDirectory = true;
            dlg.FileName = txtIneritaName.Text;
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                if (strFilePath.EndsWith(".xml"))
                {
                    List<string> lstContent = GetInertiaResultFlieContent();
                    CommonFunction.mWriteListStringToFile(strFilePath, lstContent);
                }
                ...
                if (strFilePath.EndsWith(".xls"))
                {
                    ...
                    {
                        ExportInertiaDataToExcel(strFilePath);
                    }
                }
        ...
        private void gridInertiaData_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            InertiaArithmetic model = GetInertiaData();
            if (model != null && model.FormulaList.Count > 0)
            {
                for (int i = 0; i < gridInertiaData.Rows.Count; i++)
                {
                    foreach (WeightFormula formula in model.FormulaList)
                    {
                        if (formula.NodePath == gridInertiaData.Rows[i].Cells[0].Value.ToString())
                        {
                            formula.Value = Convert.ToDouble(gridInertiaData.Rows[i].Cells[1].Value.ToString());
                        }
                    }
                }
            }
        ...
        private void btnExportInertiaIDE_Click(object sender, EventArgs e)
        {
            if (PubSyswareCom.IsRuntimeServerStarted())
            {
                for (int i = 0; i < gridInertiaData.Rows.Count; i++)
                {
                    string strName = gridInertiaData.Rows[i].Cells[0].Value.ToString();
                    double d = Convert.ToDouble(gridInertiaData.Rows[i].Cells[1].Value.ToString());

                    PubSyswareCom.CreateDoubleParameter(strName, d, true, true, false);

                    PubSyswareCom.SetParameterUnit(string.Empty, strName, "kg*m^2");
                }
