...
namespace WeightCenterDesignAndEstimateSoft.Tool
{
    public partial class ParameterManageForm : Form
    {
        ...
        private void BindTreeParaData(List<ParaData> lstPara)
        {
            ...
            if (lstParaData != null && lstParaData.Count > 0)
            {
                ...
                foreach (ParaData para in lstParaData)
                {
                    ...
                    childNode.ToolTipText = para.paraName + "|" + para.paraEnName + "|" + para.paraUnit + "|" + para.paraType + "|" + para.strRemark;
        ...
        private void SetingPageButton(string strType)
        {
            if (strType == "inti")
            {
                textBoxParaName.Enabled = false;
                textBoxParaEnName.Enabled = false;
            ...
            if (strType == "new")
            {
                textBoxParaName.Enabled = true;
                textBoxParaEnName.Enabled = true;
                ...
                textBoxParaName.Text = string.Empty;
                textBoxParaEnName.Text = string.Empty;
        ...
        private string GetPageVerification()
        {
            ...
            //参数名称
            if (textBoxParaName.Text == string.Empty)
            {
                strErroInfo = "请输入参数名称";
                return strErroInfo;
            }
            ...
            {
                if (textBoxParaName.Text.Contains(" "))
                {
                    strErroInfo = "参数名称不能包含空格";
                    return strErroInfo;
                }
                if (Verification.IsCheckString(textBoxParaName.Text))
                {
            ...
            //参数英文名称
            if (textBoxParaEnName.Text == string.Empty)
            {
                strErroInfo = "请输入参数名称";
                return strErroInfo;
            }
            ...
            {
                if (textBoxParaEnName.Text.Contains(" "))
                {
                    strErroInfo = "参数名称不能包含空格";
                    return strErroInfo;
                }
                if (Verification.IsCheckString(textBoxParaEnName.Text))
                {
                    strErroInfo = "请检查参数名称输入非法字符";
                    return strErroInfo;
                }
        ...
        private void SettingPageData()
        {
            if (selNode != null)
            {
                ...
                {
                    textBoxParaName.Text = strArray[0];
                    textBoxParaEnName.Text = strArray[1];
                    cmbUnit.Text = strArray[2];
                    cmbParameterType.SelectedIndex = int.Parse(strArray[3]);
                    txtParameterRemark.Text = strArray[4];
                }
        ...
        private List<string> GetFileContent(List<ParaData> lstParaData)
        {
            ...
            if (lstParaData != null && lstParaData.Count > 0)
            {
                ...
                for (int i = 0; i < lstParaData.Count; i++)
                {
                    ...
                    lstContent.Add(CommonFunction.mStrModifyToString8(3) + "<参数英文名称>" + lstParaData[i].paraEnName + "</参数英文名称>");
            ...
            if (result == DialogResult.Yes)
            {
                if (selNode.Level == 0)
                {
                    if (lstParaData.Count > 0 && lstAllParaData.Count > 0)
                    {
                        for (int i = 0; i < lstParaData.Count; i++)
                        {
                            for (int j = 0; j < lstAllParaData.Count; j++)
                            {
                                if (lstParaData[i].paraName == lstAllParaData[j].paraName 
                                    //&& lstParaData[i].paraType == lstAllParaData[j].paraType
                                    )
                                {
                                    lstAllParaData.Remove(lstAllParaData[j]);
                                }
                ...
                if (selNode.Level == 1)
                {
                    ...
                    for (int i = 0; i < lstTempData.Count; i++)
                    {
                        for (int j = 0; j < lstAllParaData.Count; j++)
                        {
                            if (lstTempData[i].paraName == lstAllParaData[j].paraName
                                //&& lstTempData[i].paraType == lstAllParaData[j].paraType
                                )
                            {
                                lstAllParaData.Remove(lstAllParaData[j]);
                            }
                ...
                if (selNode.Level == 2)
                {
                    ...
                    if (strArray != null && strArray.Length > 2)
                    {
                        paraType = int.Parse(strArray[3]);
                    }

                    for (int j = 0; j < lstAllParaData.Count; j++)
                    {
                        if (//paraType == lstAllParaData[j].paraType && 
                            selNode.Name == lstAllParaData[j].paraName)
                        {
                            lstAllParaData.Remove(lstAllParaData[j]);
                        }
                ...
                if (lstAllParaData.Count > 0)
                {
                    ...
                    if (fileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        for (int i = 0; i < lstTempParaData.Count; i++)
                        {
                            for (int j = 0; j < lstAllParaData.Count; j++)
                            {
                                if (//lstTempParaData[i].paraType == lstAllParaData[j].paraType && 
                                    lstTempParaData[i].paraName == lstAllParaData[j].paraName)
                                {
        ...
        private void btnCancle_Click(object sender, EventArgs e)
        {
            ...
            if (selNode.Level == 2)
            {
                textBoxParaName.Text = selNode.Name;
                string[] strArray = selNode.ToolTipText.Split('|');
        ...
        private void treeViewParameterList_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (selNode.Level == 2)
            {
                textBoxParaName.Text = e.Node.Name;
