    ...
    public partial class FromWDMCore : Form
    {
        ...
        private void FromWDM_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < airs.Length; i++)
            {
                ...
                WDMIntegrationModule.TStmc[] tstmcs = WDMIntegrationModule.GetStmcs(childNode.Name);
                for (int j = 0; j < tstmcs.Length; j++)
                {
                    TreeNode subNode = new TreeNode();
                    subNode.Name = tstmcs[j].ID;
                    subNode.Text = tstmcs[j].MC;
                    subNode.Tag = i.ToString()+j.ToString();
